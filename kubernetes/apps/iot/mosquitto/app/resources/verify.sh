...
LEGACY_RATGDO_STATUS_TOPIC="ratgdo_Small_Garage_Door/status/verify/${RUN_ID}"
LEGACY_RATGDO_COMMAND_TOPIC="ratgdo_Small_Garage_Door/command/verify/${RUN_ID}"
...
mqtt_sub "$HA_USER" "$MQTT_HA_PASSWORD" -W 10 -C 1 \
  -t "$LEGACY_RATGDO_STATUS_TOPIC" > /work/ha-legacy-ratgdo &
ha_legacy_ratgdo_subscriber=$!
sleep 1
mqtt_plain_pub "$LEGACY_RATGDO_USER" "$MQTT_RATGDO_LEGACY_PASSWORD" \
  -t "$LEGACY_RATGDO_STATUS_TOPIC" -m "$RUN_ID"
wait "$ha_legacy_ratgdo_subscriber"
grep -qx "$RUN_ID" /work/ha-legacy-ratgdo

mqtt_plain_sub "$LEGACY_RATGDO_USER" "$MQTT_RATGDO_LEGACY_PASSWORD" \
  -W 10 -C 1 -t "$LEGACY_RATGDO_COMMAND_TOPIC" > /work/legacy-ratgdo-command &
legacy_ratgdo_command_subscriber=$!
sleep 1
mqtt_pub "$HA_USER" "$MQTT_HA_PASSWORD" \
  -t "$LEGACY_RATGDO_COMMAND_TOPIC" -m "$RUN_ID"
wait "$legacy_ratgdo_command_subscriber"
grep -qx "$RUN_ID" /work/legacy-ratgdo-command
...
legacy_ratgdo_role_contract="$(ctrl getRole ha-migration-ratgdo-legacy-v1)"
printf '%s\n' "$legacy_ratgdo_role_contract" | grep -Fq 'ratgdo_Small_Garage_Door/status/#'
printf '%s\n' "$legacy_ratgdo_role_contract" | grep -Fq 'ratgdo_Small_Garage_Door/command/#'
...
expect_publish_blocked plain "$LEGACY_RATGDO_USER" "$MQTT_RATGDO_LEGACY_PASSWORD" \
