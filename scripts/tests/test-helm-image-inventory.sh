...
if ! yq -e '
  .jobs["image-pins"].strategy.matrix.include
  | map(.index)
  | join(",") == "0,1,2,3,4,5"
' "${repo_root}/.github/workflows/meta-static-analysis.yaml" >/dev/null; then
  echo 'static-analysis workflow does not define every six-way shard index exactly once' >&2
  exit 1
fi
if [[ $(yq -r '.jobs."image-pins".steps[] | select(.name == "Check rendered workload images") | .env.HELM_IMAGE_SHARD_COUNT' \
  "${repo_root}/.github/workflows/meta-static-analysis.yaml") != 6 ]]; then
...
printf 'ok: HelmRelease discovery, classified skips, concurrency, and modulo sharding validated\n'
